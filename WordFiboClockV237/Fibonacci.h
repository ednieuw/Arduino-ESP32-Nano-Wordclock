...
void PrintDisplayChoice(bool PrintIt)
{
 ...
 snprintf(sptext, sizeof(sptext),"Display choice  : %s", QchoiceNames[dp]);
...
/* Q slider tick numbers - right padding matches the swatch group width + gap below,
   so the ticks stay aligned over the (now narrower) slider track, not the swatches */
.q-ticks {
  display: flex; justify-content: space-between;
  font-size: 0.65rem; color: var(--muted);
  margin-bottom: 4px; padding: 0 92px 0 2px;
}
#qlabel { text-align: center; font-size: 0.78rem; color: var(--cyan); margin-top: 6px; }

/* Live colour swatches beside the Q slider (hours/minutes/both) - click to open the Colour Picker */
.q-row { display: flex; align-items: center; gap: 10px; }
.q-swatches { display: flex; gap: 4px; flex: none; }
.q-swatch {
  width: 24px; height: 24px; flex: none; position: relative; overflow: hidden;
  border-radius: 6px; border: 1px solid var(--border);
  cursor: pointer; transition: border-color 0.2s;
}
.q-swatch:hover { border-color: var(--cyan); }
/* Display-off indicator: a cross overlay, visible against any swatch colour via blend-mode */
.q-swatch.off::before, .q-swatch.off::after {
  content: ''; position: absolute; top: 50%; left: 50%;
  width: 140%; height: 2px; background: #fff; mix-blend-mode: difference;
}
.q-swatch.off::before { transform: translate(-50%, -50%) rotate(45deg); }
.q-swatch.off::after  { transform: translate(-50%, -50%) rotate(-45deg); }
...
<div class="card">
  ...
  <div class="q-row">
    ...
    <div class="q-swatches">
      <div id="qswatch1" class="q-swatch" onclick="location.href='/colourpick'" title="Hours colour"></div>
      <div id="qswatch2" class="q-swatch" onclick="location.href='/colourpick'" title="Minutes colour"></div>
      <div id="qswatch3" class="q-swatch" onclick="location.href='/colourpick'" title="Both colour"></div>
    </div>
  </div>
...
// -- Colour swatches (live palette colours beside the Q slider: hours/minutes/both) --
...
function setSwatch(hex1, hex2, hex3, isOn) {
  var ids = ['qswatch1', 'qswatch2', 'qswatch3'];
  var hexes = [hex1, hex2, hex3];
  for (var i = 0; i < 3; i++) {
    var el = document.getElementById(ids[i]);
    if (hexes[i]) el.style.backgroundColor = unpackToHex(hexes[i]);
    if (isOn !== undefined) el.classList.toggle('off', !isOn);   // Cross overlay when the display is off (t2: 1=on, 0=off)
  }
}
function updateSwatch() {
  fetch('/menustate').then(function(r) { return r.json(); }).then(function(s) { setSwatch(s.lc1, s.lc2, s.lc3, s.t2); }).catch(function() {});
...
function loadState() {
  fetch('/menustate')
    ...
    .then(function(s) {
      ...
      updateQlabel(qv);
      setSwatch(s.lc1, s.lc2, s.lc3, s.t2);
