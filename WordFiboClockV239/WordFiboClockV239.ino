...
#include <PubSubClient.h>      // MQTT client, used for Home Assistant discovery. https://github.com/knolleary/pubsubclient
...
// Tekstprintlnf("P: %s I: %s", PendingCommand, InputString);                                                              // To be sure Clear this to avoid it being reused from the Async webserver
 ...
 case 'V':
     if (len == 1) 
       ...
       snprintf(sptext, sizeof(sptext), "Enter Vwwrrggbb hexadecimal (0 - F)\nBackground colour Stored: 0X%08" PRIX32, Mem.DimmedLetter);
     Tekstprintln(sptext);
 ...
 if (!MQTTclient.connected())
   {
    ...
    if (MQTTclient.connect(Mem.BLEbroadcastName, MQTT_USER, MQTT_PASSWORD))
      {
...
void publishMQTTDiscovery(void)
{
 ...
 publishDiscoverySensor(deviceId, "cputemp", "CPU Temperature", baseTopic + "/cputemp", "\xC2\xB0" "C", "temperature"); // CPU Temperature sensor
 publishDiscoverySensor(deviceId, "ldr",     "LDR Reading",     baseTopic + "/ldr",     "",               "");          // Raw LDR reading, no unit/device class
 ...
 publishDiscoveryButton(deviceId, "displayonoff",   "Display On/Off",    cmndTopic, "O"); // Toggles LEDsAreOff
 ...
 publishDiscoveryButton(deviceId, "randomdisplay",   "Random Display",    cmndTopic, "~"); // Toggles Mem.RandomDisplay (cycles the display style automatically)

 publishDiscoverySelect(deviceId, "displaychoice", "Display Colour Scheme", cmndTopic, baseTopic + "/displaychoice"); // Q0-Q9 colour scheme picker
...
// Options are the QchoiceNames[] friendly names (Q0..LASTITEM, from WordClock.h/Fibonacci.h/NoClock.h) -
// HA sends the chosen name back on commandTopic; MQTTCallback() maps it back to "Qn" for ReworkInputString().
//--------------------------------------------
void publishDiscoverySelect(String deviceId, String selectKey, String friendlyName, String commandTopic, String stateTopic)
{
 String configTopic = "homeassistant/select/" + deviceId + "/" + selectKey + "/config";
 String uniqueId    = deviceId + "_" + selectKey;

 String options = "[";
 for (byte i = 0; i <= LASTITEM; i++) { if (i) options += ","; options += "\"" + String(QchoiceNames[i]) + "\""; }
 options += "]";
...
void MQTTCallback(char* topic, byte* payload, unsigned int length)
{
 ...
 for (byte i = 0; i <= LASTITEM; i++)                                                         // Is this a Display Colour Scheme name from the HA select?
   {
    if (strcmp(buf, QchoiceNames[i]) == 0) { ReworkInputString(String("Q") + i); return; }
   }
 ReworkInputString(String(buf));
}
...
//--------------------------------------------
void publishDiscoverySensor(String deviceId, String sensorKey, String friendlyName, String stateTopic, String unit, String deviceClass)
{
