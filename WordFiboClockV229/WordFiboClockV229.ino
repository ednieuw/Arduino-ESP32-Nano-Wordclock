...
// ------------------>   Derive the build variant from the selection above
  ...
  #define WORDCLOCK               // Any non-Fibonacci selection is a word clock (uses Display_WordClock.h / ClockFaces.h)
#endif
...
// CLOCK_VARIANT (build name string) is defined per-variant in Display_WordClock.h / Display_Fibonacci.h
...
#if   defined(WORDCLOCK)                                                                      // Variant-specific menu page (provides menu_html[] at /menu) 
  #include "ColourPick.h"                                                                     // Colour Picker page (WordClock)
  #include "MenuPageW.h"                                                                      // WordClock menu page at /menu
#elif defined(FIBONACCI)
  #include "MenuPageF.h"                                                                      // Fibonacci menu page
#endif
...
// --- Variant display layer (provides Displaytime() and variant helpers) --- //
#if   defined(WORDCLOCK)
  #include "Display_WordClock.h"                                                              // word-clock display logic (-> ClockFaces.h)
#elif defined(FIBONACCI)
  #include "Display_Fibonacci.h"
#endif
  ...
  switch(toupper(InputString[0])) 
   {
    ...
    case 'Q':                                                                                 // Display choice
      if(len == 1)                                                                            // Print display options menu   
       {
        for (byte i = 0; i <= LASTITEM; i++) Tekstprintlnf("  Q%d= %s", i, QchoiceNames[i]); // Names per variant (Display_WordClock.h / Display_Fibonacci.h)
 ...
 server.on("/menustate", HTTP_GET, [](AsyncWebServerRequest *request)                         // Return current Mem toggle states as JSON
  {
   ...
   snprintf(json, sizeof(json),
     ...
     "\"noff\":%d,\"non\":%d,\"ds3231hw\":%d,\"rtcsrc\":\"%s\",\"kldr\":%d,\"kmin\":%d,\"khour\":%d,\"koff\":%d}",
   ...
   snprintf(json, sizeof(json),
     ...
     "\"noff\":%d,\"non\":%d,"
     "\"ds3231hw\":%d,\"rtcsrc\":\"%s\",\"kldr\":%d,\"kmin\":%d,\"khour\":%d,\"koff\":%d}",
