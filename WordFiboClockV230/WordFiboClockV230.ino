...
//   FIBONACCI selects the Fibonacci clock; any other selection is a word clock.
...
// The ONEWIRE's are a nice to have options. I leave them as a define for sometimes use
...
#define WORDCLOCK                                                                             // Any non-Fibonacci selection is a word clock (uses WordClock.h / ClockFaces.h)
             #endif
             #if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL (3, 3, 0)                     // Use EdSoftLED with ESP32 compiler.
#define USEEDSOFTLED  // Use EdsoftLED >= V1.9.0 for SK6812. Saves 200-500 bytes compared with NeoPixel. To use NeoPixel is also OK but compile: By GPIO number (legacy)
             #endif
             #ifdef USEEDSOFTLED
