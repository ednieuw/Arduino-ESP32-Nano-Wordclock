/* 
 ...
 Changes V229: New Name WordFiboClock. Combined WordclockV228 and FibonacciV011. The HC-12 / BLE TimeSender is a new feature (H06). 
 Changes V230: Added Fibo colourpicker. Combined headers. Error with Neopixel wrong pinnumbering.
 ...
 Changes V236: Added: MQTT CheckMQTT() non-blocking — only runs when Mem.WIFIcredentials == SET_AND_OK and WiFi is actually connected;
 Changes V237: Added: MQTT - HA buttons Y, ~, O, P, @ and Q1-9, Two test sensors: CPU Temperature and LDR Reading, published once/minute. 
...
Board: ESP32 core version >3.3.10 
Partition Scheme: With FAT
Pin Numbering: By Arduino pin (default)     -- > with EdsoftLED both can be used
               By GPIO number (legacy)      -- > When using NEOpixel   Adafruit_NeoPixel/esp.c:78:(.text.espShow+0x55): undefined reference to `digitalPinToGPIONumber' when using By Arduino pin
USB mode: Normal (Tiny USB)
**********************
*/
// =============================================================================================================================
// *******  Select below, with only one #define selected, the clock variant *******
...
//   FIBONACCI selects the Fibonacci clock; 
//#define FIBONACCI               // Fibonacci / chrono clock

//   NOCLOCK you can delete Clockfaces.h,WordClock.h and Fibonacci.h 
// #define NOCLOCK
// ------------------>   Derive the build variant from the selection above

//------------------------------------------------------------------------------              //
// ESP32 Definition of installed modules
// The ONEWIRE's are a nice to have options. I leave them as a define for sometime use
...
//-------------------------------------------
             #if !defined(FIBONACCI) && !defined(NOCLOCK)
#define WORDCLOCK                                                                             // Any non-Fibonacci, non-NOCLOCK selection is a word clock (uses WordClock.h / ClockFaces.h)
             #endif
             #if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL (3, 3, 0)                     // Use EdSoftLED with ESP32 compiler.
#define USEEDSOFTLED  // Use EdsoftLED >= V1.10.0 for SK6812. Saves 200-500 bytes compared with NeoPixel. To use NeoPixel is also OK but compile: By GPIO number (legacy)
             #endif
             #ifdef USEEDSOFTLED
#include <EdSoftLED.h>         // For LED strip WS2812 or SK6812. https://github.com/ednieuw/EdSoftLED 
             #else
#include <Adafruit_NeoPixel.h> // Compile By GPIO number (legacy) https://github.com/adafruit/Adafruit_NeoPixel
             #endif
//------------------------------------------------------------------------------              //
// Build-environment sanity check - stop the build on an easy-to-miss IDE setting
//--------------------------------------------
             #if !defined(USEEDSOFTLED) && defined(BOARD_HAS_PIN_REMAP)                       // NeoPixel + Tools > Pin Numbering = 'By Arduino pin'
  #error "NeoPixel build with 'Pin Numbering = By Arduino pin'. Set Tools > Pin Numbering to 'By GPIO number (legacy)' (and use GPIO numbers, e.g. LED_PIN = 8) to avoid the 'undefined reference to digitalPinToGPIONumber' link error."
             #endif

#include <Preferences.h>       // For storage
...
#include <RTClib.h>            // Used for connected DS3231 RTC. https://github.com/adafruit/RTClib
#include <Encoder.h>           // For rotary encoder. https://github.com/PaulStoffregen/Encoder
#include <Keypad.h>            // For 3x1 membrane keypad by Mark Stanley & Alexander Brevig 
#include <IRremote.hpp>        // IR remote control. https://github.com/Arduino-IRremote/Arduino-IRremote
#include <PubSubClient.h>      // MQTT client, used for Home Assistant discovery. https://github.com/knolleary/pubsubclient PubSubClient3 by Nick O'Leary in library manager

// CLOCK_VARIANT (build name string) is defined per-variant in WordClock.h / Fibonacci.h
//------------------------------------------------------------------------------              //
// SPIFFS storage
//--------------------------------------------
...
const byte DEFAULTCOLOUR = 0;                                                                 // Generic default (used by Reset - shared)
...
enum DigitalPinAssignments {      // Digital hardware constants
 ...
 HC12_TX      = D4,               // D4 HC-12 433MHz time sender (TX only). Shares D4 with the IR receiver - cannot use both at once
 HC12_RX      = D6,               // D6 HC-12 Feel free to connect the HC-12 to other unused pins D9, D10 for instance
 ...
 PCB_LED_D10  = D10,              // D10
 EmptyD11     = D11,              // D11 
 COLPIND12    = D12,              // D12 Used as dummy with KEYPAD
...
struct    EEPROMstorage {                                                                     // Data storage in EEPROM to maintain them after power loss
  ...
  byte HC12Time         = 0;                                                                  // Use HC12 timesender
  byte ResetPin         = 0;                                                                  // Use Resetpin on GPIO number
  ...
  uint32_t DimmedLetter = 0;                                                                  // Self defined "dimmed character"/Chrono colour for word/fibo clock display
  ...
  char Timezone[50];
  int  Checksum         = 0;
  uint32_t ColourNL     = 0;                                                                  // Self defined NL/Off colour for word/fibo clock display
  uint32_t ColourUK     = 0;                                                                  // Self defined UK/hours colour for word/fibo clock display
  uint32_t ColourDE     = 0;                                                                  // Self defined DE/minutes colour for word/fibo clock display
  uint32_t ColourFR     = 0;                                                                  // Self defined FR/both colour for word/fibo clock display  
...
#include "Webpage.h"                                                                          // The Clock web page (shared)
#include "SoftAP.h"                                                                           // The web page to enter SSID and password of the WIFI router (shared)
#include "OTAhtml.h"                                                                          // OTA update page (shared)
#include "LogViewer.h"                                                                        // Log viewer page (shared)
...
//----------------------------------------
const char* MQTT_SERVER        = "homeassistant.local";                                       // Mosquitto broker, resolved via mDNS (needs MDNS.begin(), already called in StartWIFI_NTP)
const uint16_t MQTT_PORT       = 1883;
const char* MQTT_USER          = "mqtt_user";
const char* MQTT_PASSWORD      = Mem.Password;
WiFiClient   MQTTWifiClient;
PubSubClient MQTTclient(MQTTWifiClient);
bool MQTTdiscoveryPublished    = false;                                                       // Discovery is (re)published once per MQTT connection
const char* const RandomChoiceNames[3] = {"Off", "Per minute", "Per hour"};                   // Options of the HA Random Display select; index = Mem.RandomDisplay
unsigned long MQTTlastAttempt  = 0;                                                           // millis() of last (re)connect attempt
unsigned long MQTTlastLoopCall = 0;                                                           // millis() of last client.loop()/connected() check - both hit the TCP socket, throttle away from the hot loop()
//----------------------------------------                                                    //
// WPS
//----------------------------------------
...
// --- Variant code: display layer + colour-picker page + menu page --------- //
// One file per variant (provides Displaytime(), ColourPick_html[], menu_html[]).
#if   defined(WORDCLOCK)
  #include "WordClock.h"                                                                      // WordClock display + picker + menu (-> ClockFaces.h)
#elif defined(FIBONACCI)
  #include "Fibonacci.h"                                                                      // Fibonacci display + picker + menu
#elif defined(NOCLOCK)
  #include "NoClock.h"                                                                        // No display: no-op stubs only (headless BLE/HC-12 build)
#endif

// ===========================================================================
//  TIME SENDER  (HC-12 + BLE SERVER MODE) - shared, runtime-gated by
//  Mem.TimeSender (BLE) and Mem.HC12Time (HC-12). Enable with the H06 command.
// ===========================================================================
static NimBLEServer*           TSpServer           = nullptr;
static NimBLEService*          TSpService          = nullptr;
static NimBLECharacteristic*   TSpTX               = nullptr;
static NimBLECharacteristic*   TSpRX               = nullptr;
static NimBLEAdvertising*      TSpAdvertising      = nullptr;
...
static bool                    TimeSenderServerRunning = false;
static unsigned long           TSsendDelayms       = 0;
static bool                    TSdatePending       = false;
...
void setup() 
{
 ...
 InitStatusLEDs();                                                                            // Init status LEDs
 ...
 if(Mem.TimeInput==1) {LEDstartup(pink);   InitRotaryMod();         Tekstprintln("Rotary started"); }            // Start the Rotary encoder
 if(Mem.TimeInput==2) {LEDstartup(grass);  InitKeypad3x1();         Tekstprintln("Keypad started"); }            // Start the Keypad 3x1 
 if(Mem.TimeInput >2) {LEDstartup(white);  Start_IRreceiver();      Tekstprintln("IRremote started");}           // Start IR remote Mem.TimeInput = 3 or 4                                                                                     
 ...
 if(Mem.WIFIOn)       {LEDstartup(purple); ConnectWIFI();           Tekstprintln("WIFI started"); }              // Start WIFI and optional NTP if Mem.WIFIOn = 1 
                      {LEDstartup(yellow); InitTimeSystem();        Tekstprintln("Time system started");}        // Initialize RTC + DS3231 sync
 ...
 if(Mem.TimeSender)   {LEDstartup(sky);    BLETimeSenderStart();    Tekstprintln("Time Sender started"); }   // Start the BLE TimeSender (H06)
 if(Mem.HC12Time)     {LEDstartup(amber);  InitHC12();              Tekstprintln("HC12 Time started"); }     // Start the HC-12 time sender
 if(Mem.ResetPin)  {LEDstartup(orangered); InitResetpin();          Tekstprintlnf("RST button on GPIO %d started",Mem.ResetPin); } //  
 ...
 Tekstprint("Compiled on: "); Tekstprintln(ARDUINO_BOARD);                                    //  #ifdef ARDUINO_BOARD 
 Tekstprintlnf("Software version: %s", SoftwareName());
 Tekstprintlnf("Clock variant: %s",CLOCK_VARIANT);
 ...
 if (msLeap > 999)                                                                            // Every second enter the loop
  {
   ...
   if(Mem.ResetPin) CheckResetPin();                                                          // Check  reset button
  if (Mem.TimeReceiver) CheckTimeReceiverClient();                                            // Check if a TimeSender is in the air
  if (Mem.TimeSender)   CheckTimeSender();                                                    // H06: push time/date over BLE TimeSender (self-throttled to 10s)
   DimLeds(TestLDR);                                                                          // Every second an intensity check and update from LDR reading
   if (shouldReboot)                                                                          // After finish OTA update restarts after 3 seconds. 1 second is too fastú
     {
      if (MQTTclient.connected()) MQTTclient.publish((MQTTDeviceId() + "/status").c_str(), "offline", true); // Tell HA we are going down instead of waiting for the MQTT timeout
...
void EveryMinuteUpdate(void)
{
 ...
 if(Mem.HC12Time && Mem.TimeLogPrint == 1) SendHC12TimeString();                              // HC-12 broadcast per minute (K1). K2 = per hour, K0 = off
...
void EveryHourUpdate(void)
{
 ...
 if(Mem.HC12Time && Mem.TimeLogPrint == 2) SendHC12TimeString();                              // HC-12 broadcast per hour (K2). K1 = per minute, K0 = off
...
//--------------------------------------------                                                //
// COMMON Init status LEDs
//-------------------------------------------- 
void InitStatusLEDs(void)
{
 analogWriteResolution(LED_RED,   9);                                                         // Pin resolution/frequency explicitly, before the first analogWrite() on these pins -
 analogWriteResolution(LED_GREEN, 9);                                                         // 9-bit (max 511) matches SetStatusLED()'s own 0-512 intensity scale directly, and the
 analogWriteResolution(LED_BLUE,  9);                                                         // core's implicit default for this has already silently changed across ESP32 core versions once before
 analogWriteFrequency(LED_RED,   5000);
 analogWriteFrequency(LED_GREEN, 5000);
 analogWriteFrequency(LED_BLUE,  5000);
 SetStatusLED(0,10,0);                                                                        // Set to green
 }
...
// Analog range 0 - 511. 0 is LED Off, 511 is max intensity
// Resolution is pinned to 9-bit in setup() (analogWriteResolution) to match this scale directly,
// regardless of the core's own implicit default, which has shifted across ESP32 core versions before.
//--------------------------------------------
void SetStatusLED(int Red, int Green, int Blue)
{
 analogWrite(LED_RED,   511 - Red);
 analogWrite(LED_GREEN, 511 - Green);
 analogWrite(LED_BLUE,  511 - Blue);
}
//--------------------------------------------                                                //
...
//--------------------------------------------                                                //
// COMMON Initialyse Resetpin when installed
// Restarting the Arduino 5 times in the hour also resets de settings to default or Rotary press 9 times
//--------------------------------------------
void InitResetpin(void)
{
  if(Mem.ResetPin) pinMode(Mem.ResetPin, INPUT_PULLUP);                                       // Pull RST_PIN pin high if pin >0  
}
//--------------------------------------------
// COMMON  Check reset button on RST_PIN (D6)
// Triggers Reset() and ResetCredentials(); when held LOW for >= 2 seconds.
// Called once per second from EverySecondCheck().
//--------------------------------------------
void CheckResetPin(void)
{
 if(!Mem.ResetPin)  return;                                                                   // if(Mem.ResetPin)==0 then no resetpin
 static uint8_t PressedSeconds = 0;
 if (digitalRead(RST_PIN) == HIGH) {PressedSeconds = 0; return;}                              // Button not pressed
 if (++PressedSeconds >= 2)
  {
   Tekstprintln("Reset pressed. Settings to Factory default. Restarting");   
   Reset();
   ResetCredentials();
   shouldReboot = true;                                                                       // Causes ESP.restart(); 
  }
}
...
void Reset(void)
{
 Mem.Checksum         = 25065;                                                                // Check number control validity of the struct
 ...
 Mem.Ringbufcnt       = 0;                                                                    // Display the ringbuffer counter in the logging
 Mem.HC12Time         = 0;                                                                    // Use HC12 timesender
 Mem.ResetPin         = RST_PIN;                                                              // Use a Reset button on D2 or D6
 Mem.byteFuture1      = 0;                                                                    // For future use
 Mem.byteFuture2      = 0;                                                                    // For future use
 Mem.MQTT             = 0;                                                                    // Turn On or Off MQTT 
...
{
 GetStructFromFlashMemory();                                                                  // General parameters
...
// if(strlen(Mem.Password)<5 || strlen(Mem.SSID)<3)     Mem.WIFIOn = Mem.NTPOn = 0;           // If ssid or password invalid turn WIFI/NTP off
 ...
 StoreStructInFlashMemory();                                                                  // if settings were out of range store them properly
}

//--------------------------------------------                                                //
// COMMON Store Mem.struct in FlashStorage 
//--------------------------------------------
...
void StoreIRRemoteInFlashMemory(void)                                                         // Save IR remote settings separately
{
...
//--------------------------------------------                                                //
// COMMON Get Mem data from FlashStorage
//--------------------------------------------
void GetStructFromFlashMemory(void)
{
 FLASHSTOR.begin("Mem", false);
 bool ok = FLASHSTOR.getBytes("Mem", &Mem, sizeof(Mem) );
 FLASHSTOR.end(); 
 Tekstprintlnf("Get Mem data Struct %s: Mem.Checksum = %d",ok?"OK":"NOK",Mem.Checksum);
}
...
void GetIRRemoteFromFlashMemory(void)
{
 ...
 bool ok = FLASHSTOR.getBytes("IRRemote", &IRMem, sizeof(IRMem));
 FLASHSTOR.end();
 Tekstprintlnf("Get IRremote data Struct %s",ok?"OK":"NOK");
}
...
// Tekstprintlnf("P: %s I: %s", PendingCommand, InputString);                                 // To be sure Clear this to avoid it being reused from the Async webserver
  ...
  switch(toupper(InputString[0])) 
   {
    ...
    case 'F':
     if (InputString.equalsIgnoreCase("FF"))    // FF / ff toggles Fibonacci <-> Chrono display
        {
         Mem.FiboChrono = !Mem.FiboChrono;
         snprintf(sptext, sizeof(sptext), "Display is %s", Mem.FiboChrono ? "Fibonacci" : "Chrono");
         Tekstprintln(sptext);
         lastminute = 99; Displaytime(); Tekstprintln("");    // Force a display update
         sptext[0] = 0;
         break;
        }
      if (ParseColourInput(InputString, len, Mem.OwnColour))                                   // SetSecondColour() below re-applies the colour, so no LetterColor assignment needed here
    ...
    case 'H':                                                                                 // Use rotary encoder
      if(len == 3) 
       {
        byte Choice = (byte)SConstrainInt(InputString, 2, 0, 6);                              // keep the range between 0 and 6; H00 - H06
        if      (Choice <  5) Mem.TimeInput = Choice;                                         // H00-H04 time input method
        else if (Choice == 5) Mem.TimeReceiver = 1 - Mem.TimeReceiver;                        // H05 receive time over BLE
        else if (Choice == 6) { Mem.TimeSender = 1 - Mem.TimeSender;                          // H06 send time (HC-12 / BLE)
                                if (Mem.TimeSender) { Mem.TimeReceiver = 0; Mem.TimeInput = 0; } } // a sender is not a receiver and uses NTP (H00)
        if(Mem.TimeInput > 4)  Mem.TimeInput = 0;
        Tekstprintln("\n **** Restart with @ to activate changed setting ****");    // 
      ...
      else Tekstprintln("Error **** Enter H00 (none/NTP), H01=Rotary, H02=Membrane, H03 large IR, H04 tiny IR, H05 Time receiver, H06 Time sender ****");
      sprintf(sptext, "\n Use of rotary encoder is %s\n"
                        "         Use of keypad is %s\n"
                        "Use of large IR-remote is %s\n"
                        " Use of tiny IR-remote is %s\n"
                        "  Time receiver (H05) is %s\n"
                        "    Time sender (H06) is %s",
                        Mem.TimeInput == 1 ? "ON" : "OFF", Mem.TimeInput == 2 ? "ON" : "OFF",
                        Mem.TimeInput == 3 ? "ON" : "OFF", Mem.TimeInput == 4 ? "ON" : "OFF",
                        Mem.TimeReceiver   ? "ON" : "OFF", Mem.TimeSender ? "ON" : "OFF");
      Tekstprintln(sptext);      
    ...
    case 'O':                                                                                 // Turn On/Off Display. O toggles, O1 = on, O0 = off (used by the HA switch)
      if(len == 1 || (len == 2 && (InputString[1] == '0' || InputString[1] == '1')))
       {
        if(len == 1) LEDsAreOff = !LEDsAreOff;
        else         LEDsAreOff = (InputString[1] == '0');
    ...
    case 'Q':                                                                                 // Display choice
      if(len == 1)                                                                            // Print display options menu   
       {
        for (byte i = 0; i <= LASTITEM; i++) Tekstprintlnf("  Q%d= %s", i, QchoiceNames[i]); // Names per variant (WordClock.h / Fibonacci.h)
 ...
 case 'V':
     if (len == 1) snprintf(sptext, sizeof(sptext),"Cleared background colour: 0X%08" PRIX32, Mem.DimmedLetter = 0);  // Reset to no background LEDs
     else if (ParseColourInput(InputString, len, Mem.DimmedLetter)) snprintf(sptext, sizeof(sptext),"**** Background font colour changed ****\nColour now: 0X%08" PRIX32, Mem.DimmedLetter);
     else  snprintf(sptext, sizeof(sptext),"Enter Vwwrrggbb hexadecimal (0 - F)\nBackground colour Stored: 0X%08" PRIX32, Mem.DimmedLetter);
     Tekstprintln(sptext);
      ...
      if(len == 1) 
       {
        ...
        wpsInitConfig();
        wpsStart();
        snprintf(sptext, sizeof(sptext),"Starting WPS");        
       }
      break;
    ...
    case '%':                                                                                 // LED strip type
     ...
     case '^':  
      if(len > 1 && len < 4) 
       {
        Mem.ResetPin = (byte)SConstrainInt(InputString, 1, 0, 99);
        snprintf(sptext, sizeof(sptext), "Reset button on GPIO pin: %d", Mem.ResetPin);
       } 
      else snprintf(sptext, sizeof(sptext), "**** Input fault. \nEnter between 0 and 99");
      break;
    ...
    case '}':                                                                                 // Turn On printing ringbuffer size bytes and counter position
      if(len == 1) 
        {
    ...
    case '~':                                                                                 // Turn On 
      if(len == 1) 
       {
        if (Mem.RandomDisplay > 0) Mem.RandomDisplay = 0;                                     // ~ change per minue toggle
        else                       Mem.RandomDisplay = 1;
       }
      if(len == 2) 
       {
        byte ff = (byte)InputString.substring(1, 2).toInt();
        if (ff < 3) Mem.RandomDisplay = ff;                                                   // ~1 change per minute, ~2 per hour
       }
...
void PrintMem(void)
{
 ...
 Tekstprintlnf("N TurnOnLEDsAtHH: %d",Mem.TurnOnLEDsAtHH); 
...
Tekstprintlnf("%%      LED strip: %s",Mem.LEDstrip?"WS2812":"SK6812" );    
 ...
 Tekstprintlnf("^ RST BTN @ GPIO: %s",Mem.ResetPin ? String(Mem.ResetPin).c_str() : "0 (= OFF)");
 Tekstprintlnf("     byteFuture1: %d",Mem.byteFuture1 );  
 Tekstprintlnf("     byteFuture2: %d",Mem.byteFuture2 );                                                             
 ...
 // Tekstprintlnf("B            : %s",Mem.Password);                           
 ...
 Tekstprintlnf(".     Own colour: 0X%08" PRIX32, Mem.OwnColour); 
 Tekstprintlnf("       Colour NL: 0X%08" PRIX32, Mem.ColourNL); 
 Tekstprintlnf("       Colour UK: 0X%08" PRIX32, Mem.ColourUK); 
 Tekstprintlnf("       Colour DE: 0X%08" PRIX32, Mem.ColourDE); 
 Tekstprintlnf("       Colour FR: 0X%08" PRIX32, Mem.ColourFR); 
 Tekstprintlnf("    DimmedLetter: 0X%08" PRIX32, Mem.DimmedLetter); 
 Tekstprintlnf("          Extra1: %d",Mem.Extra1 );  
 Tekstprintlnf("          Extra2: %d",Mem.Extra2 ); 
//  Tekstprintlnf("Mem ControlNumber: %d",Mem2.ControlNumber); 
 sptext[0]=0;                                                                                 // Suppress a second print of sptext
}
...
void ReadAndAverageLDR(bool clean)
{
 ...
 if(Mem.DimmedLetter && LEDNEARLDR < 9999)                                                    // Now the LDR is influenced by the LED near to it. Turn it off, settle for a second and read the LDR
   {
...
//--------------------------------------------
uint32_t StripGetPixelColor(uint32_t Lednr)
{
 ...
 if (LEDColour == CLEARLEDSTARTUP)  
   {
    for(int n = ProgressLedNr-1; n>=0; n--)
      {
       uint32_t Co =StripGetPixelColor(n);
       SetStatusLED(Cred(Co),Cgreen(Co),Cblue(Co));                                           // is of no use        
       ColorLed(n,0); 
       ShowLeds(); 
       delay(150);
      }
...
//--------------------------------------------                                                //
// LED Clear an oversized test strip 
// When testing a clock build with < 256 LEDs on a 256-LED strip, the surplus LEDs keep whatever they held after upload. 
// Grow the buffer to 256, blank the whole strip, then shrink back to NUM_LEDS so the clock runs at its real size.
//--------------------------------------------
...
// ARDUINO_EVENT_WIFI_STA_STOP → WiFi off
       ...
       case ARDUINO_EVENT_WIFI_STA_DISCONNECTED:
            ...
            //WiFi.reconnect(); //is checked in EveryHourUpdate() via CheckRestoreWIFIconnectivity()
            break;
...
//--------------------------------------------                                                //
// MQTT Check connection and maintain the MQTT client (non-blocking)
//--------------------------------------------
void CheckMQTT(void)
{
 if (!Mem.MQTT) return; 
 ...
 if (!MQTTclient.connected())
   {
    MQTTdiscoveryPublished = false;                                                           // Force a fresh discovery publish after the next successful connect
    if (millis() - MQTTlastAttempt < 5000) return;                                            // Do not block the loop, retry every 5 sec
    MQTTlastAttempt = millis();
    MQTTclient.setServer(MQTT_SERVER, MQTT_PORT);
    MQTTclient.setBufferSize(1024);                                                           // Default 256 bytes is too small for the HA discovery JSON; the "select" entity's 10-option list alone needs ~514 bytes total packet size
    MQTTclient.setCallback(MQTTCallback);
    Tekstprintln("[MQTT] Connecting...");
    String statusTopic = MQTTDeviceId() + "/status";                                          // Availability topic: broker publishes "offline" (the Last Will) when the clock drops off
    if (MQTTclient.connect(Mem.BLEbroadcastName, MQTT_USER, MQTT_PASSWORD, statusTopic.c_str(), 0, true, "offline"))
      {
       Tekstprintln("[MQTT] Connected");
       MQTTclient.publish(statusTopic.c_str(), "online", true);                               // Mark the device available again
      ...
      {
       Tekstprintlnf("[MQTT] Connect failed, rc=%d", MQTTclient.state());
      }
    return;
   }
 MQTTclient.loop();
...
//--------------------------------------------                                                //
// MQTT Sanitize BLEbroadcastName for use in topic strings (no spaces/slashes)
//--------------------------------------------
String MQTTDeviceId(void)
{
 String deviceId = String(Mem.BLEbroadcastName);
 deviceId.replace(" ", "_");
 deviceId.replace("/", "_");
...
//--------------------------------------------                                                //
// MQTT Call once after MQTT connects successfully, but only if WIFI credentials are valid
//--------------------------------------------
void publishMQTTDiscovery(void)
{
 if (Mem.WIFIcredentials != SET_AND_OK || MQTTdiscoveryPublished) return;

 String deviceId  = MQTTDeviceId();
 String baseTopic = deviceId;                                                                 // e.g. "wordclock_livingroom"
 ...
 publishDiscoverySensor(deviceId, "mcurestarts",  "MCU Restarts",       baseTopic + "/mcurestarts",  "", "",          true);  // Mem.MCUrestarted - restarts during setup()
 publishDiscoverySensor(deviceId, "wifireboots",  "WiFi Reboots",       baseTopic + "/wifireboots",  "", "",          true);  // Mem.WIFInoConnection - reboots after 30h without IP
 publishDiscoverySensor(deviceId, "looprestarts", "Loop-stall Reboots", baseTopic + "/looprestarts", "", "",          true);  // Mem.LoopRebooted - reboots after a starving main loop
 if (DS3231Installed)
    publishDiscoverySensor(deviceId, "ds3231temp", "DS3231 Temperature", baseTopic + "/ds3231temp", "\xC2\xB0" "C", "temperature", false); // Temperature register of the DS3231 RTC module

 String cmndTopic = baseTopic + "/cmnd";                                                      // Shared command topic - every button/switch publishes its single-letter command here
 publishDiscoveryButton(deviceId, "ledtest",        "LED Test",          cmndTopic, "Y"); // Runs the existing LEDtest() command as a proof that MQTT commands reach ReworkInputString()
 publishDiscoveryButton(deviceId, "statusledonoff", "StatusLED On/Off",  cmndTopic, "P"); // Toggles Mem.StatusLEDOn
 publishDiscoveryButton(deviceId, "restart",        "Restart",           cmndTopic, "@"); // Reboots the ESP32

 MQTTclient.publish(("homeassistant/button/" + deviceId + "/displayonoff/config").c_str(),  "", true); // Delete the retained configs of the old buttons,
...
//--------------------------------------------                                                //
// MQTT Generic helper: publishes one MQTT Discovery config for a button.
// The button always sends payloadPress to commandTopic, which MQTTCallback() forwards straight into ReworkInputString() -
// the same single-letter command parser already used by BLE/Serial/the web page.
//--------------------------------------------
void publishDiscoveryButton(String deviceId, String buttonKey, String friendlyName, String commandTopic, String payloadPress)
{
 String configTopic = "homeassistant/button/" + deviceId + "/" + buttonKey + "/config";
 String uniqueId    = deviceId + "_" + buttonKey;

 String payload  = "{";
 payload += "\"name\":\""          + friendlyName  + "\",";
 payload += "\"command_topic\":\""+ commandTopic  + "\",";
 payload += "\"payload_press\":\""+ payloadPress  + "\",";
 payload += "\"availability_topic\":\"" + deviceId + "/status\",";                            // Entity shows 'unavailable' when the clock is offline (Last Will)
 payload += "\"unique_id\":\""     + uniqueId      + "\",";
 payload += "\"device\":{";
 payload +=   "\"identifiers\":[\"" + deviceId + "\"],";
 payload +=   "\"name\":\""         + String(Mem.BLEbroadcastName) + "\",";
 payload +=   "\"manufacturer\":\"EdNieuw\",";
 payload +=   "\"model\":\"WordClock\"";
 payload += "}}";
...
//--------------------------------------------                                                //
// MQTT Generic helper: publishes one MQTT Discovery config for a select (dropdown of options).
// optionNames is any list of friendly names (e.g. QchoiceNames[] or RandomChoiceNames[]) -
// HA sends the chosen name back on commandTopic; MQTTCallback() maps it back to a command for ReworkInputString().
//--------------------------------------------
void publishDiscoverySelect(String deviceId, String selectKey, String friendlyName, String commandTopic, String stateTopic, const char* const optionNames[], byte optionCount)
{
 String configTopic = "homeassistant/select/" + deviceId + "/" + selectKey + "/config";
 String uniqueId    = deviceId + "_" + selectKey;

 String options = "[";
 for (byte i = 0; i < optionCount; i++) { if (i) options += ","; options += "\"" + String(optionNames[i]) + "\""; }
 options += "]";
...
//--------------------------------------------                                                //
// MQTT Generic helper: publishes one MQTT Discovery config for a switch (on/off with state).
// HA sends payloadOn/payloadOff to commandTopic (existing single-letter commands); the clock
// reports the real state on stateTopic ("ON"/"OFF") via PublishMQTTStates().
//--------------------------------------------
void publishDiscoverySwitch(String deviceId, String switchKey, String friendlyName, String commandTopic, String stateTopic, String payloadOn, String payloadOff)
{
 String configTopic = "homeassistant/switch/" + deviceId + "/" + switchKey + "/config";
 String uniqueId    = deviceId + "_" + switchKey;
 ...
 payload += "\"payload_off\":\""  + payloadOff    + "\",";
...
//--------------------------------------------                                                //
// MQTT Generic helper: publishes one MQTT Discovery config for a number (slider).
// commandTemplate wraps the slider value in an existing menu command, e.g. "M{{ value }}"
// makes HA publish "M200", which goes straight into ReworkInputString().
//--------------------------------------------
void publishDiscoveryNumber(String deviceId, String numberKey, String friendlyName, String commandTopic, String stateTopic, int minVal, int maxVal, String commandTemplate)
{
 String configTopic = "homeassistant/number/" + deviceId + "/" + numberKey + "/config";
 String uniqueId    = deviceId + "_" + numberKey;
 ...
 payload += "\"min\":" + String(minVal) + ",\"max\":" + String(maxVal) + ",\"step\":1,";
 payload += "\"availability_topic\":\"" + deviceId + "/status\",";                            // Entity shows 'unavailable' when the clock is offline (Last Will)
 payload += "\"unique_id\":\""        + uniqueId        + "\",";
 payload += "\"device\":{";
 payload +=   "\"identifiers\":[\"" + deviceId + "\"],";
...
void PublishMQTTStates(void)
{
 ...
 itoa(Mem.MCUrestarted, buf, 10);           MQTTclient.publish((base + "/mcurestarts").c_str(),   buf, true);
 itoa(Mem.WIFInoConnection, buf, 10);       MQTTclient.publish((base + "/wifireboots").c_str(),   buf, true);
...
//--------------------------------------------                                                //
// MQTT Incoming command callback - forwards the raw payload into the existing single-letter
// command parser (ReworkInputString), the same one BLE/Serial/the web page already use.
// A payload matching one of QchoiceNames[] (from the HA select entity) is translated to "Qn" first.
//--------------------------------------------
void MQTTCallback(char* topic, byte* payload, unsigned int length)
{
 if (length == 0 || length > 40) return;                                                     // ReworkInputString() itself caps input at 40 chars
 char buf[41];
 memcpy(buf, payload, length);
 buf[length] = 0;
 Tekstprintlnf("[MQTT] Command received on %s: %s", topic, buf);
...
//--------------------------------------------                                                //
// MQTT Generic helper: publishes one MQTT Discovery config for a sensor
//--------------------------------------------
void publishDiscoverySensor(String deviceId, String sensorKey, String friendlyName, String stateTopic, String unit, String deviceClass, bool diagnostic)
{
 String configTopic = "homeassistant/sensor/" + deviceId + "/" + sensorKey + "/config";
 String uniqueId    = deviceId + "_" + sensorKey;

 String payload  = "{";                                                                       // Build JSON payload manually (small enough not to need ArduinoJson here)
 payload += "\"name\":\""                + friendlyName + "\",";
 payload += "\"state_topic\":\""         + stateTopic   + "\",";
 if (unit.length())        payload += "\"unit_of_measurement\":\"" + unit        + "\",";    // Omit when empty - HA rejects an empty device_class/unit
 if (deviceClass.length()) payload += "\"device_class\":\""        + deviceClass + "\",";
 if (diagnostic)           payload += "\"entity_category\":\"diagnostic\",";                  // Shown in HA's Diagnostic section instead of Sensors
 payload += "\"availability_topic\":\""  + deviceId     + "/status\",";                       // Entity shows 'unavailable' when the clock is offline (Last Will)
...
#if defined(WORDCLOCK) || defined(FIBONACCI)
 server.on("/colourpick", HTTP_GET, [](AsyncWebServerRequest *request)
  {
  String page = ColourPick_html;
  char buf[12];
  page.replace("%%CNAME%%", Mem.BLEbroadcastName);                                            // Page title = broadcast name
  snprintf(buf, sizeof(buf), "%08" PRIX32, Mem.ColourNL);     page.replace("%%COLNL%%", buf);
  snprintf(buf, sizeof(buf), "%08" PRIX32, Mem.ColourDE);     page.replace("%%COLDE%%", buf);
  snprintf(buf, sizeof(buf), "%08" PRIX32, Mem.ColourFR);     page.replace("%%COLFR%%", buf);
  snprintf(buf, sizeof(buf), "%08" PRIX32, Mem.ColourUK);     page.replace("%%COLUK%%", buf);
  snprintf(buf, sizeof(buf), "%08" PRIX32, Mem.DimmedLetter); page.replace("%%COLBG%%", buf);
  request->send(200, "text/html", page);
  });
#endif   // colour picker (WordClock + Fibonacci)
#if defined(WORDCLOCK) || defined(FIBONACCI)
 server.on("/menu", HTTP_GET, [](AsyncWebServerRequest *request)                              // Serve the settings menu page
   {  request->send(200, "text/html", menu_html);  });
#endif   // settings menu (WordClock + Fibonacci)

 server.on("/menustate", HTTP_GET, [](AsyncWebServerRequest *request)                         // Return current Mem toggle states as JSON
  {
   char json[512];
#if defined(FIBONACCI)
   byte dp = _min(Mem.DisplayChoice, LASTITEM);  // Selected palette row - used below for the 3 live swatch colours
   snprintf(json, sizeof(json),
     "{\"tf\":%d,\"t2\":%d,\"t3\":%d,\"t5\":%d,\"t8\":%d,\"tj\":%d,\"t9\":%d,"
     "\"hi\":\"h0%d\",\"qval\":%d,\"sval\":%d,\"lval\":%d,\"mval\":%d,"
     "\"noff\":%d,\"non\":%d,\"ds3231hw\":%d,\"rtcsrc\":\"%s\",\"kldr\":%d,\"kmin\":%d,\"khour\":%d,\"koff\":%d,\"cname\":\"%s\","
     "\"lc1\":\"%08" PRIX32 "\",\"lc2\":\"%08" PRIX32 "\",\"lc3\":\"%08" PRIX32 "\"}",
     Mem.FiboChrono   ? 1 : 0,  // tf F  Fibonacci display
     LEDsAreOff       ? 0 : 1,  // t2 O
     ...
     (int)Mem.DisplayChoice,    // qval Q0-Q9
     ...
     DS3231Installed  ? 1 : 0,  // ds3231hw
     (Mem.UseDS3231 && DS3231Installed) ? "DS3231" : (Mem.NTPOn ? "NTP" : "ESP32"),
     TestLDR              ? 1 : 0,  // kldr K  (LDR test)
     ...
     colors[dp][1],                 // lc1 hours colour  - swatch 1
     colors[dp][2],                 // lc2 minutes colour - swatch 2
     colors[dp][3]);                // lc3 both colour   - swatch 3
#else   // WORDCLOCK
   snprintf(json, sizeof(json),
    ...
    switch (NoofRotaryPressed)                                                                // No of times the rotary is pressed
      {
       ...
       case 7:                                                      break;                    //                                
       case 8:                                                      break;
       case 9: { Reset(); ResetCredentials(); shouldReboot = true;} break;                    // Reset all settings                                                                  
      default:                                                      break;                     
      }
